...
<link rel="stylesheet" type="text/css"
	href="<%=ctxPath%>/styles/product_styles.css">
<link rel="stylesheet" type="text/css"
	href="<%=ctxPath%>/styles/product_responsive.css">
...
.middle-guide-shipping-info, .middle-guide-cancle-info,
	.middle-guide-refund-info {
	margin-bottom: 30px;
}
	...
	$(document)
			.ready(
					function() {
						...
						$(".accordion_banner .accordion_title")
								.click(
										function() {
											if ($(this).next("div").is(
													":visible")) {
												...
												$(
														".accordion_banner .accordion_sub")
														.slideUp("fast");
												$(this).next("div")
														.slideToggle("fast");
											}
						...
						var bannerOffset = $('.productView-middle-declare')
								.position();
								...
								.submit(
										function() {
											...
											if (user_id == '') {
												...
												var select = $(
														'#select-ask-kind option:selected')
														.val();
												var lock = document
														.getElementById('ask-lock');
						...
						$('.answer-add')
								.click(
										function(event) {
											...
											var pbr_id = document
													.getElementById('pbr_id_'
															+ num).value;
											var pbr_head = document
													.getElementById('pbr_head_'
															+ num).value;
											var pbr_content = document
													.getElementById('pbr_content_'
															+ num).value;
											var p_num = document
													.getElementById('pbr_p_num_'
															+ num).value;
											var brand_num = document
													.getElementById('pbr_brand_num_'
															+ num).value;
													...
													.ajax({
														...
														success : function(data) {
															if (data.result == 'logout') {
																...
																alert('답변이 완료되었습니다!');
																location
																		.replace("/Hwahea/shop/shopProductView.do?p_num="
																				+ p_num
																				+ "&brand_num="
																				+ brand_num);
															} else {
		...
		<div class="container" style="margin-left: 0px;">
			<div class="productView-middle-select">
				<div class="middle-select-tab">
					<ul>
						<li class="select-tab-intro tab-selected" data-target="intro">상품
							소개</li>
			...
			<div class="productView-middle-preview">
				<span class="middle-preview-content"> 상품 설명 Content상품 설명
					Content상품 설명 Content상품 설명 Content<br> 상품 설명 Content상품 설명
					Content상품 설명 Content상품 설명 Content<br> 상품 설명 Content상품 설명
					Content상품 설명 Content상품 설명 Content<br> 상품 설명 Content상품 설명
					Content상품 설명 Content상품 설명 Content<br> 상품 설명 Content상품 설명
					Content상품 설명 Content상품 설명 Content<br> 상품 설명 Content상품 설명
					Content상품 설명 Content상품 설명 Content<br>
				</span>
			</div>
				...
				<div class="middle-guide-shipping-info">
					...
					<div class="guide-content">
						<ul>
							...
							<li>배송예정일 : 결제완료 후 1~3일 후 출고됩니다. 단, 상품의 재고 상황 및 운송업체 사정에 따라
								배송이 지연될 수 있습니다.</li>
							<li class="guide-addition">* 제주도를 포함한 도서산간 지역은 추가 배송일과 추가
								배송비 입금 요청이 있을 수 있습니다.</li>
						</ul>
					</div>
				</div>
				...
				<div class="middle-guide-refund-info">
					...
					<div class="guide-content">
						<ul>
							...
							<li>반품 배송비 : 단순 변심에 의한 환불은 고객이 부담, 상품 불량 및 오배송에 의한 환불은 판매자
								부담</li>
							...
							<li class="guide-addition">* 미성년자가 체결한 계약은 법정대리인이 동의하지 않는 경우
								본인 또는 법정대리인이 취소할 수 있습니다.</li>
						</ul>
					</div>
				</div>
			</div>
			<div class="productView-middle-ask scorll-target" id="select-tab-ask"
				data-selected="ask">
				<div class="middle-ask-content">
					<div class="col-md-12 ask-content-top">
						<div class="col-md-8 ask-content-top-left">
							...
							<p class="ask-content-subTitle">문의하신 내용은 판매자가 확인 후 친절히 답변해드려요
								:)</p>
						</div>
						...
						<c:forEach var="ask" items="${boardList}">
							<c:if test="${user_auth != 5}">
								<div class="col-md-12 ask-question-and-answer accordion_banner"
										...
										<div class="col-md-8 question-left">
											<p style="margin-bottom: 15px;">
												<c:if test="${ask.pbr_content eq null}">
													<span class="question-mark" style="background: #E89C0C;">답변대기</span>
												</c:if>
												<c:if test="${ask.pbr_content ne null}">
													<span class="question-mark">질문</span>
												</c:if>
												<span class="question-date"> <fmt:formatDate
														value="${ask.pb_dateTime}" pattern="yy.MM.dd HH:mm:ss" /></span>
												...
												<c:if test="${user_id == ask.pb_id}">
													<span class="question-buttons" style="margin-left: 5px;">
														<input type="button" value="삭제" class="question-remove"
														onclick="location.href='#'">
													</span>
												</c:if>
											</p>
											<c:if
												test="${ask.pb_lock==0 || (ask.pb_lock==1&&(ask.pb_id==user_id))}">
												<p>
											...
											<c:if test="${ask.pb_lock==1 && (ask.pb_id!=user_id)}">
												<p id="private-content-${ask.pb_num}"
									...
									<div
										class="col-md-12 ask-content-answer accordion_sub private-${ask.pbr_head}"
										id="private-${ask.pbr_head}">
											...
											<c:if test="${ask.pbr_content ne null}">
												<p style="margin-bottom: 15px;">
													<span class="answer-mark">답변완료</span> <span
														class="answer-date"><fmt:formatDate
															value="${ask.pbr_dateTime}" pattern="yy.MM.dd HH:mm:ss" /></span>
												</p>
							...
							<c:if test="${user_auth == 5}">
								<div class="col-md-12 ask-question-and-answer accordion_banner"
									...
									<div class="col-md-12 ask-content-question accordion_title">
										<div class="col-md-8 question-left">
											<p style="margin-bottom: 15px;">
												...
												<span class="question-date"><fmt:formatDate
														value="${ask.pb_dateTime}" pattern="yy.MM.dd HH:mm:ss" /></span>
												| <span class="question-nickName">${ask.pb_id} 님</span> <span
													class="question-buttons" style="margin-left: 5px;">
													<input type="button" value="삭제" class="question-remove"
													onclick="location.href='#'">
												</span>
											</p>
											<p>
												<span class="question-kind">[<c:if
														test="${ask.pb_lock==1 && (ask.pb_id!=user_id)}">비밀글:</c:if>${ask.pb_kind}]</span>
									...
									<div
										...
										<div class="answer-right">
											<c:if test="${ask.pbr_content eq null}">
												<input type="hidden" id="pbr_p_num_${ask.pb_num}"
													name="pbr_p_num_${ask.pb_num}" value="${product.p_num}">
												<input type="hidden" id="pbr_brand_num_${ask.pb_num}"
													name="pbr_brand_num_${ask.pb_num}"
													value="${product.brand_num}">
												<input type="hidden" id="pbr_id_${ask.pb_num}"
													name="pbr_id_${ask.pb_num}" value="${user_id}">
												<input type="hidden" id="pbr_head_${ask.pb_num}"
													name="pbr_head_${ask.pb_num}" value="${ask.pb_num}">
												<span class="answer-mark" style="background: #aaa;">WAITING</span>
												<p class="add-edit-form">
													<textarea id="pbr_content_${ask.pb_num}"
														name="pbr_content_${ask.pb_num}"
														placeholder="답변 대기중인 문의글입니다."></textarea>
													<span class="letter-count">400/400</span>
												</p>
												<p style="margin-bottom: 15px;" class="question-buttons">
													<input type="button" value="답변등록" class="answer-add"
														data-private="${ask.pb_num}"> <input type="button"
														value="수정" class="answer-edit" onclick="location.href='#'"
														data-private="${ask.pb_num}">
												</p>
											</c:if>
											<c:if test="${ask.pbr_content ne null}">
												<p style="margin-bottom: 15px;" class="question-buttons">
													...
													<c:if test="${user_id == 'admin'}">
														<input type="button" value="수정" class="question-remove"
															onclick="location.href='#'">
													</c:if>
												</p>
												<p class="answer-content">${ask.pbr_content}</p>
					...
					<div class="col-md-12 ask-content-end">${pagingHtml}</div>
				</div>
			</div>
